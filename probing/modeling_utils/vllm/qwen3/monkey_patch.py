from vllm.model_executor.models.qwen3 import Qwen3ForCausalLM, Qwen3Model, Qwen3DecoderLayer, Qwen3Attention
from vllm.distributed import get_pp_group, get_tensor_model_parallel_world_size, get_tensor_model_parallel_rank
from vllm.model_executor.models.qwen3 import IntermediateTensors
...
def steering_attention_forward(
    ...
    if self.steering_mode == "before_o_proj":
        if hasattr(self, 'valid_hyperplane_vector_dict') and \
            steering_flag is not None and \
            layer_idx is not None and \
            layer_idx in self.valid_hyperplane_vector_dict.keys():
                # current head rank is
                ...
                for i, (head_idx, steering_vector) in enumerate(steering_vector_dict.items()):
                    if head_idx >= head_start and head_idx < head_end:
                        ...
                        awareness = attn_output[steering_flag, dim_start:dim_end] @ hyperplane.T
                        awareness = awareness.sigmoid() # (k, 1) # 0->non-linear, 1->linear

                        # if self.steering_mode == 'dev':
                        #     # weakening the non-linear part, hard-version
                        #     awareness = 2 * (awareness > 0.5).to(attn_output.dtype) - 1
                        ...
                        attn_output[steering_flag, dim_start:dim_end] += self.steering_coef * add_vector
...
def steering_model_forward(
    ...
    for idx, layer in enumerate(
        ...
        hidden_states, residual = layer(positions, hidden_states, residual, idx, steering_flag)
...
def causal_lm_forward(
    ...
    if self.if_steering and context is not None and context.attn_metadata is not None:
        if context.attn_metadata['model.layers.0.self_attn.attn'].max_query_len > 1:
            ...
            # if input_ids == self.steering_think_end_id:
...
def monkey_patch_qwen3_vllm(steering_vector_path, steering_number, steering_coef, steering_mode, model_name_or_path):
    ...
    Qwen3Attention.valid_hyperplane_vector_dict = valid_hyperplane_vector_dict
    Qwen3Attention.steering_coef = steering_coef
    Qwen3Attention.steering_mode = steering_mode # dev, soft
    
    Qwen3Attention.forward = steering_attention_forward

    Qwen3DecoderLayer.forward = steering_layer_forward
    
    Qwen3Model.forward = steering_model_forward

    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
    vocab = tokenizer.get_vocab()

    Qwen3ForCausalLM.new_round = False
    Qwen3ForCausalLM.steering_think_flag = None

    Qwen3ForCausalLM.steering_split_ids = torch.LongTensor([vocab[token] for token in vocab.keys() if "ĊĊ" in token]).to(device)
    Qwen3ForCausalLM.steering_think_start_id = tokenizer.encode("<think>", add_special_tokens=False)[0]
    Qwen3ForCausalLM.steering_think_end_id =  tokenizer.encode("</think>", add_special_tokens=False)[0]

    Qwen3ForCausalLM.logit_adjustment_tokens = None
    Qwen3ForCausalLM.logit_adjustment_values = None
    Qwen3ForCausalLM.logit_adjustment_position = None
    Qwen3ForCausalLM.logit_adjustment_max_len = None

    Qwen3ForCausalLM.if_steering = True

    Qwen3ForCausalLM.set_steering_flag = set_steering_flag
    Qwen3ForCausalLM.forward = causal_lm_forward
